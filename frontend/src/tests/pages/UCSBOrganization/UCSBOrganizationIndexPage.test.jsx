...
describe("UCSBOrganizationIndexPage tests", () => {
  ...
  test("renders three ucsborganizations correctly for regular user", async () => {
    ...
    const orgTranslationShort = screen.getByText("Baking Club");
    expect(orgTranslationShort).toBeInTheDocument();

    const orgTranslation = screen.getByText("Baking Club at UCSB");
  ...
  test("what happens when you click delete, admin", async () => {
    ...
    axiosMock
      ...
      .reply(200, "UCSBOrganization with id BAKE deleted");
    ...
    await waitFor(() => {
      expect(mockToast).toBeCalledWith("UCSBOrganization with id BAKE deleted");
    });
